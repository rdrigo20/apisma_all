<?php
// Função de callback para deletar um card por slug news_details
function delete_news_details_by_slug(WP_REST_Request $request) {
    ...
    return new WP_REST_Response(array('message' => 'news_details deletado'), 200);
}

// Função para registrar o endpoint
function registrar_delete_news_details_by_slug() {
    register_rest_route('api', '/news_details/(?P<slug>[a-zA-Z0-9-]+)', array(
        'methods' => 'DELETE',
        'callback' => 'delete_news_details_by_slug',
    ));
}
add_action('rest_api_init', 'registrar_delete_news_details_by_slug');
?>

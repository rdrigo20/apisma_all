...
function get_events_by_slug(WP_REST_Request $request) {
...
function registrar_get_events_by_slug() {
    register_rest_route('api', '/events/(?P<slug>[a-zA-Z0-9-]+)', array(
        'methods' => 'GET',
        'callback' => 'get_events_by_slug',
    ));
}
add_action('rest_api_init', 'registrar_get_events_by_slug');
